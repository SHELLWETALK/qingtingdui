...
typedef struct {
  ...
  __IO uint32_t MOD;                               // Modulo, offset: 0x8   //？？模式？
  struct {                                         // offset: 0xC, array step: 0x8 
      __IO uint32_t CnSC;                               // Channel (n) Status and Control, array offset: 0xC, array step: 0x8 
    __IO uint32_t CnV;                               // Channel (n) Value, array offset: 0x10, array step: 0x8
  } CONTROLS[8];
  __IO uint32_t CNTIN;                             // Counter Initial Value, offset: 0x4C   //计数器初值
  __I  uint32_t STATUS;                            // Capture and Compare Status, offset: 0x50 
  __IO uint32_t MODE;                              // 功能模式选择, offset: 0x54 
  ...
  __IO uint32_t FILTER;                            // Input Capture Filter Control, offset: 0x78   //输入
  __IO uint32_t FLTCTRL;                           // Fault Control, offset: 0x7C      
  __IO uint32_t QDCTRL;                            // Quadrature Decoder Control and Status, offset: 0x80       //正交解码计数控制与状态
  __IO uint32_t CONF;                              // Configuration, offset: 0x84 //设置
  __IO uint32_t FLTPOL;                            // FTM Fault Input Polarity, offset: 0x88  //ftm错误输入... 
