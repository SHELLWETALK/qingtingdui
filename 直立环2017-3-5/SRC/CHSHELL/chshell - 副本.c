    ...
    if(*str == 'm' || *str == 'M')
    {
        ...
        if(*str == 'a' || *str == 'A')
        {
            ...
            if(*str == 'n' || *str == 'N')
            {
                ...
                if(*str == ' ')
                {
                    CHS_MemCopy(str,FunName,4); //拷贝FunName字符串
                    str++;
...
//          6长度超出最大，在拷贝过后
    if(Result == 2)
    {
       for(i=0;i<CHS_Dev.fnum;i++)
        {
            Result=CHS_StrCheck((uint8_t*)CHS_Dev.funs[i].name,LocalFunName,LocalFunParm);//得到本地函数名及参数
            if(CHS_StrCmp(LocalFunName,RecFunParm))     //输入函数名和本地函数名匹配
            {
                UART_printf("man ");
                UART_printf("%c",RecFunParm );
                UART_printf("\r\n");
                UART_printf("%c",(uint8_t*)CHS_Dev.funs[i].man );
                UART_printf("\r\n");
                return 0;
            }
        }            
    }
    else if(Result!=0) //不是C语言函数格式的输入
	{
		...
		{					   
			...
			case 1://帮助指令
                ...
                UART_printf("man:   各个函数的帮助信息\r\n");
				UART_printf("请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");
                UART_printf("系统命令不需要按照程序编写格式输入\r\n");
				UART_printf("--------------------------YANDLD ------------------------- \r\n");
			case 2:
				UART_printf("\r\n");
				UART_printf("-------------------------函数清单--------------------------- \r\n");
	  ...
	  //检查本地是否有此函数
		...
		for(i=1;i<CHS_Dev.pnum+1;i++) //输出要执行的参数
		{
			if(CHS_Dev.PInfo.Type[i]==1) //数字
			{
